...
ggplot(gwg2020, aes(time_in_game, goal_number)) +
  ...
  theme_bw() + 
  labs(title="Game Winning Goal by Goal Types", colour = "Goal Type" ) +
  #separate into 3 graphs for clearer graphs since > 400 games
  facet_wrap(~ goal_type)

ggplot(gwg2020, aes(time_in_game, goal_number, shape = goal_type, colour = goal_type )) +
  geom_point() +
...
ggplot(fullgame, aes(x=additional_goals, fill = half, color=half)) +
  ...
  geom_vline(data=fullgame_mean, aes(xintercept=grp.mean, color=half),
             linetype="dashed") +
  scale_color_manual(values=c("#4197D9", "#660066", "#9966CC")) +
  scale_fill_manual(values=c("#4197D9", "#660066", "#9966CC")) +
